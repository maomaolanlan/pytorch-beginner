...
# many to many 结构，给一些列输入得到一系列输出，
# 这种结构可用于翻译或聊天对话场景，对输入的文本转换成另外一些列文本。
# 此例为词性分析（名词，动词，助词等）
# http://www.pytorchtutorial.com/10-minute-pytorch-9/
...
tag_to_idx_ops = {v : k for k, v in tag_to_idx.items()}
    ...
    def forward(self, x, word):
        ...
        for each in word:
            ...
            if torch.cuda.is_available():
                tempchar = self.char_lstm(char_list.cuda())
            else:
                tempchar = self.char_lstm(char_list)
...
print()
input_str="Everybody ate the apple".split()
input = make_sequence(input_str, word_to_idx)
if torch.cuda.is_available():
    input = input.cuda()

out = model(input, input_str)
print(out)
_, pred = torch.max(out, 1)
print(pred)

for index,val in enumerate(pred):
    print("{:<10}\t{}".format(input_str[int(index)],tag_to_idx_ops[int(val)]))
